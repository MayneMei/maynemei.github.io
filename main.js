(function () {
  ...
  var TEXT_SCORE = "被气了 %s 次";
  var TEXT_GAME_OVER = "你被恩沛气了%s次\n";
  var TEXT_TRY_AGAIN = "再爱我一次嘛";
  var TEXT_PLAY_BGM = "放歌";
  var TEXT_TIME_ELAPSED = "忍了 %s 秒";
  var TEXT_TOTAL_TIME_ELAPSED = "累计忍了 %s 秒";
  var TEXT_TINY_TIPS = "让我多活一会儿，求求宝贝了";
  ...
  var _pipes, _pipeInvisibleLines, _pipesTimer;

  var _frog;

  var _ground;

  var _clouds, _cloudsTimer;
  ...
  var _bgmKeyCode = [Phaser.Keyboard.ZERO, Phaser.Keyboard.NUMPAD_0];

  var _flapKeyCode = [Phaser.Keyboard.E, Phaser.Keyboard.SPACEBAR];

  var _feedbackKeyCode = [Phaser.Keyboard.FIVE, Phaser.Keyboard.NUMPAD_5];

  var _feedback, _feedbackFunc, _feedbackText, _feedbackSprite;

  var _loadingText, _tinyTipsText;

  var _timeElapsedText, _startTime, _timeElapsed;
  ...
  function spawnPipes() {
    ...
    var pipeY =
      (_game.height - 16 - o() / 2) / 2 +
      ((Math.random() > 0.5 ? -1 : 1) * Math.random() * _game.height) / 6;
    // Bottom pipe
  ...
  function initGround() {
    _ground = _game.add.tileSprite(
      0,
      _game.world.height - 32,
      _game.world.width,
      32,
      "ground"
    );
  ...
  function playHurtSound() {
    if (_currentScoreSound) _currentScoreSound.stop();
    randomPlaySound(_hurtSounds, _numHurtSounds);
  }

  function playFlapSound() {
    if (!_flapSound.isPlaying) _flapSound.play();
  }
  ...
  function showGameOver() {
    ...
    _totalTimeElapsedText.setText(
      TEXT_TOTAL_TIME_ELAPSED.replace("%s", _totalTimeElapsed)
    );
    _totalTimeElapsedText.renderable = true;

    var a = Math.floor((_score / _timeElapsed) * 100);
    a = TEXT_GAME_OVER.replace("%s", _score)
      .replace("%s", _timeElapsed)
      .replace("%s", a);
  ...
  function initFeedback() {
    if (!_feedback) return;
    _feedbackText = _game.add.text(0, 0, _feedback, {
      font: "14px " + TEXT_FONT,
      fill: "#fff",
      stroke: "#430",
      strokeThickness: 4,
  ...
  function initTexts() {
    ...
    _playBgmText = _game.add.text(0, 0, TEXT_PLAY_BGM, {
      font: "14px " + TEXT_FONT,
      fill: "#fff",
      stroke: "#430",
      strokeThickness: 4,
    ...
    _totalTimeElapsedText = _game.add.text(_game.world.width / 2, 0, "", {
      font: "14px " + TEXT_FONT,
      fill: "#f00",
      align: "center",
    });
  ...
  function init(options) {
    if (typeof options.debug !== "undefined") _debug = options.debug;

    if (typeof options.gravity !== "undefined") _gravity = options.gravity;

    if (typeof options.speed !== "undefined") _speed = options.speed;

    if (typeof options.flap !== "undefined") _flap = options.flap;

    if (typeof options.spawnRate !== "undefined")
      _spawnRate = options.spawnRate;

    if (typeof options.opening !== "undefined") _opening = options.opening;
    ...
    if (typeof options.baseUrl !== "undefined") _baseUrl = options.baseUrl;

    if (typeof options.feedback !== "undefined") _feedback = options.feedback;
